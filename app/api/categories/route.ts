  ...
  try {
    ...
    try {
      ...
      if (store_id) {
        ...
        if (error) console.error("[/api/categories] Supabase error (store_id):", error.message);
      ...
      if (store_slug) {
        const { data: storeRow, error: storeErr } = await supabase
          ...
          .maybeSingle();
        if (storeErr) {
          console.warn("[/api/categories] store lookup warning:", storeErr.message);
        } else if (storeRow?.id) {
      ...
      if (!resolvedId) {
        const { data: firstStore, error: firstErr } = await supabase
          ...
          .maybeSingle();
        if (firstErr) {
          console.warn("[/api/categories] first-store fallback warning:", firstErr.message);
      ...
      if (error) {
        console.error("[/api/categories] Supabase error (resolved store):", error.message);
        return Response.json({ categories: [] });
      }
      ...
      console.error("[/api/categories] Query error:", innerErr?.message ?? innerErr);
      return Response.json({ categories: [] });
    }
  } catch (err: any) {
    console.error("[/api/categories] Unexpected error:", err?.message ?? err);
    return Response.json({ categories: [] });
  }
}
