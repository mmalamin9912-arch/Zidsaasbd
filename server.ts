...
async function resolveStoreSlugByRef(ref: string): Promise<string | null> {
  ...
  const { supabaseUrl, supabaseKey, isConfigured } = getServerSupabaseConfig();
  if (!clean || !isConfigured) return null;
  try {
    if (isUuidLike(clean)) {
      const sbRes = await fetch(`${supabaseUrl}/rest/v1/stores?id=eq.${encodeURIComponent(clean)}&select=store_slug&limit=1`, {
        headers: { 'apikey': supabaseKey, 'Authorization': `Bearer ${supabaseKey}` }
  ...
  } catch (e) { console.warn('[Server] resolveStoreSlugByRef warning:', e); }
  return null;
}

async function resolveStoreIdBySlug(storeSlug: string): Promise<string | null> {
  const { supabaseUrl, supabaseKey, isConfigured } = getServerSupabaseConfig();
  if (!storeSlug || !isConfigured) return null;
  try {
    const sbRes = await fetch(`${supabaseUrl}/rest/v1/stores?store_slug=eq.${encodeURIComponent(storeSlug)}&select=id&limit=1`, {
      headers: { 'apikey': supabaseKey, 'Authorization': `Bearer ${supabaseKey}` }
    });
    if (sbRes.ok) {
      const rows = await sbRes.json();
